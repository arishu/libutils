...
namespace libutilscore.FTP
{
    public class SharpFTP
    {
        ...
        {
            string[] retValue = { "False", "没有获取到执行结果" };
        ...
        public static Tuple<bool, string> GetExecResult(string operationId)
        {
            ...
            try
            {
                ...
                Log.Logger.Info("Response result from server: {0}", responseStr);

                string[] result = ParseResponse(responseStr);

                ret = Tuple.Create(bool.Parse(result[0]), "");
            }
            catch (Exception ex)
            {
                Log.Logger.Error("Failed to get result associated with operationId '{0}': {1}", operationId, ex.Message);
